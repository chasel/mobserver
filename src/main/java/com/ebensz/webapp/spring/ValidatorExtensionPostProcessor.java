package com.ebensz.webapp.spring;
 ...
 * add validation configuration for their own persistent classes to an MobServer application without modifying any of the
 * existing MobServer Spring configuration or jar distribution files.

 * <p>As an example consider the following Spring bean configuration:

 * <pre>
 * &lt;bean class=&quot;com.ebensz.webapp.spring.ValidatorExtensionPostProcessor&quot;&gt;
 ...
 * process coupled with MobServer's configuration file auto detection will ensure that the validation extension is
 * automatically included into the application without requiring any modification of MobServer's existing config files.
 *
